...
# Settings
TARGET = 'block'  # block, block-stim, stim-side. reward or choice
...
NUM_SPLITS = 5
CHANCE_LEVEL = 'phase-rand'  # pseudo-blocks, phase-rand, shuffle or none
...
for i in range(len(sessions)):
        ...
        for r, region in enumerate(np.unique(regions)):
            ...
            if TARGET == 'block-stim':
                ...
                for k in range(ITERATIONS):
                    ...
                    # Decode
                    decode_this = decode(spks_region, clus_region, trial_times, trial_ids,
                                         pre_time=PRE_TIME, post_time=POST_TIME,
                                         classifier=DECODER, cross_validation=VALIDATION,
                                         num_splits=NUM_SPLITS)
                    decode_result = decode_result.append(decode_this, ignore_index=True)
                decode_result = decode_result.mean()
            else:
                # Decode
    ...
    decoding_result.to_pickle(join(SAVE_PATH,
                    ('%s_%s_%s_%s_%s_%s_cells.p' % (DECODER, TARGET, CHANCE_LEVEL,
                                                    VALIDATION, INCL_SESSIONS, INCL_NEURONS))))
