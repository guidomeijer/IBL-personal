...
for i in range(N_SESSIONS):
    ...
    decode_block = decode(spks_region, clus_region, trial_times, trial_ids,
                          pre_time=PRE_TIME, post_time=POST_TIME,
                          classifier=DECODER, cross_validation=CROSS_VALIDATION,
                          num_splits=5)
    phase_block = decode(spks_region, clus_region, trial_times, trial_ids,
                         pre_time=PRE_TIME, post_time=POST_TIME,
                         classifier=DECODER, cross_validation=CROSS_VALIDATION,
                         num_splits=5, phase_rand=True, iterations=ITERATIONS)

    shuffle_block = decode(spks_region, clus_region, trial_times, trial_ids,
                           pre_time=PRE_TIME, post_time=POST_TIME,
                           classifier=DECODER, cross_validation=CROSS_VALIDATION,
    ...
    block_trans = np.append([0], np.array(np.where(np.diff(trial_ids) != 0)) + 1)
    block_trans = np.append(block_trans, [trial_ids.shape[0]])
    for j, trans in enumerate(block_trans[:-1]):
        p = Rectangle((trans, -0.05), block_trans[j+1] - trans, 1.1, alpha=0.5,
                      color=block_colors[trial_ids[trans]])
        ax2.add_patch(p)
    ax2.plot(np.convolve(decode_block['predictions'][0], np.ones(3), 'same') / 3, lw=2, color='k')

    ax2.set(xlim=[0, trial_ids.shape[0]], ylim=[-0.05, 1.05],
            ylabel='Block prediction (5 trial rolling average)', xlabel='Trials',
