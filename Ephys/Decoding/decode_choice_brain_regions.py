...
for i in range(len(sessions)):
   ...
   # Extract eid based on data format
    ...
    if (((np.sum(trial_ids == 0) / np.sum(trial_ids == 1)) > 0.9)
            or ((np.sum(trial_ids == 0) / np.sum(trial_ids == 1)) < 0.1)):
    ...
    decoding_result.to_pickle(join(SAVE_PATH,
       ('decode_choice_%s_%s_neurons_%s_sessions.p' % (DECODER, INCL_NEURONS, INCL_SESSIONS))))
