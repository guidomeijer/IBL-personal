...
decoding_plot = decoding_result[(decoding_result['r_mean_prior'] >= MIN_R)
...
if FULL_NAME:
    ...
    sns.barplot(x='r_over_chance_prior', y='full_region', data=decoding_plot,
                ...
                ax=ax1)
    sns.swarmplot(x='r_over_chance_prior', y='full_region', data=decoding_plot,
...
ax1.set(xlabel='Decoding improvement over chance (r)', ylabel='',
        xlim=[0, YLIM])
ax1.set(title='Regression of exponentially smoothed stimulus prior from pre-stim activity')


ax2.hist(decoding_result['r_mean_prior'])
ax2.set(ylabel='Recordings', xlabel='Decoding improvement over chance (r)')

ax3.hist(decoding_result['r_prior_null'])
ax3.set(ylabel='Recordings', xlabel='Chance level decoding (r)')

ax4.scatter(decoding_result['r_mean_prior'], decoding_result['r_mean_block'])
ax4.plot([-0.1, 0.5], [-0.1, 0.5], color='k', lw=2)
_, p = ttest_rel(decoding_result['r_mean_prior'], decoding_result['r_mean_block'])
