...
for p, probe in enumerate(channels.keys()):
    ...
    if (isdir(join(FIG_PATH, 'StimOn', '%s_%s' % (SUBJECT, DATE)))
            and (OVERWRITE is True)):
        shutil.rmtree(join(FIG_PATH, 'StimOn', '%s_%s' % (SUBJECT, DATE)))
    if not isdir(join(FIG_PATH, 'StimOn', '%s_%s' % (SUBJECT, DATE))):
        mkdir(join(FIG_PATH, 'StimOn', '%s_%s' % (SUBJECT, DATE)))
        for n, cluster in enumerate(sig_units):
            ...
            plt.xlabel('Time (s)')
            plt.tight_layout()
            plt.savefig(join(FIG_PATH, 'StimOn', '%s_%s' % (SUBJECT, DATE),
        ...
        print('%d out of %d units are significantly responsive to reward delivery' % (
                                    len(sig_units), len(np.unique(spikes[probe].clusters))))
        if (isdir(join(FIG_PATH, 'Reward', '%s_%s' % (SUBJECT, DATE)))
                and (OVERWRITE is True)):
            shutil.rmtree(join(FIG_PATH, 'Reward', '%s_%s' % (SUBJECT, DATE)))
        if not isdir(join(FIG_PATH, 'Reward', '%s_%s' % (SUBJECT, DATE))):
            mkdir(join(FIG_PATH, 'Reward', '%s_%s' % (SUBJECT, DATE)))
            for n, cluster in enumerate(sig_units):
                fig, ax = plt.subplots(1, 1)
                sns.set(style="ticks", context="paper", font_scale=2)
                bb.plot.peri_event_time_histogram(spikes[probe].times, spikes[probe].clusters,
                ...
                plt.xlabel('Time (s)')
                plt.tight_layout()
                plt.savefig(join(FIG_PATH, 'Reward', '%s_%s' % (SUBJECT, DATE),
                                 '%s_n%d' % (region, cluster)))
                plt.close(fig)
        ...
        print('%d out of %d units are significantly responsive to reward omission' % (
                                    len(sig_units), len(np.unique(spikes[probe].clusters))))
        if (isdir(join(FIG_PATH, 'RewardOmission', '%s_%s' % (SUBJECT, DATE)))
                and (OVERWRITE is True)):
            shutil.rmtree(join(FIG_PATH, 'RewardOmission', '%s_%s' % (SUBJECT, DATE)))
        if not isdir(join(FIG_PATH, 'RewardOmission', '%s_%s' % (SUBJECT, DATE))):
            mkdir(join(FIG_PATH, 'RewardOmission', '%s_%s' % (SUBJECT, DATE)))
            for n, cluster in enumerate(sig_units):
                fig, ax = plt.subplots(1, 1)
                sns.set(style="ticks", context="paper", font_scale=2)
                bb.plot.peri_event_time_histogram(spikes[probe].times, spikes[probe].clusters,
                ...
                plt.xlabel('Time (s)')
                plt.tight_layout()
                plt.savefig(join(FIG_PATH, 'RewardOmission', '%s_%s' % (SUBJECT, DATE),
                                 '%s_n%d' % (region, cluster)))
                plt.close(fig)
