...
for i, session_start in enumerate(sessions_start):
    ...
    timing_check.to_pickle(join(data_path, 'timing_check_all'))
    
timing_check = pd.read_pickle(join(data_path, 'timing_check_all'))
...
ax1.scatter(timing_check['session_start_time'], failure_rate)
ax1.set(ylabel='Failure rate (%)', xlim=[datetime.date(2019,2,15), datetime.date(2019,8,1)])
ax1.plot([datetime.date(2019,7,10), datetime.date(2019,7,10)], [-0.2, 100], 'g')
#ax1.plot([datetime.date(2019,7,10), datetime.date(2019,8,1)],[5,5], 'r')
plt.setp(ax1.xaxis.get_majorticklabels(), rotation=40)

ax2.scatter(timing_check['session_start_time'], failure_rate)
ax2.set(ylabel='Failure rate (%)', xlim=[datetime.date(2019,7,10), datetime.date(2019,8,1)])
ax2.plot([datetime.date(2019,7,10), datetime.date(2019,8,1)],[5,5], 'r')
plt.setp(ax2.xaxis.get_majorticklabels(), rotation=40)

plt.tight_layout(pad = 3)
plt.savefig(join(fig_path, 'timing_checks_all_failure.pdf'), dpi=300)
plt.savefig(join(fig_path, 'timing_checks_all_failure.png'), dpi=300)
...
plt.tight_layout(pad = 3)
plt.savefig(join(fig_path, 'timing_checks_all.pdf'), dpi=300)
plt.savefig(join(fig_path, 'timing_checks_all.png'), dpi=300)
