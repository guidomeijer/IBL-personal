...
from dlc_functions_basic import load_dlc_training, load_event_times, load_events, px_to_mm
from dlc_functions_plotting import peri_plot
from dlc_functions_analysis import pupil_features

# Query sessions with available DLC data using ONE
one = ONE()
dtypes = ['_ibl_leftCamera.dlc', '_ibl_leftCamera.times'
          'trials.feedback_times', 'trials.feedbackType',
          'trials.stimOn_times', 'trials.choice']
eids = one.search(dataset_types=dtypes)

# Loop over sessions
for i, eid in enumerate(eids):
    dlc_dict, dlc_times, feedback_times, feedback_type, stimon_times, choice = one.load(
            eid, dataset_types=dtypes)


  #  data = one.load(eid, dataset_types=dtypes, dclass_output=True)
