...
for i, eid in enumerate(eids):
    ...
    if ((spikes is None) or (len(spikes) == 0) or (not hasattr(trials, 'stimOn_times')) or
            (len(trials.probabilityLeft) != len(trials.contrastLeft))):
        continue
...
resp.to_csv(join(DATA_PATH, 'contra_stim_units_map.csv'))
